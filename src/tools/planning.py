...
def setup_planning_tools(mcp: FastMCP, spec_manager: SpecificationManager):
    ...
    async def generate_implementation_plan(
        ...
        try:
            ...
            if not success:
                return {"status": "error", "message": "Failed to generate implementation plan"}
    ...
    async def update_implementation_plan(
        ...
        try:
            ...
            if not success:
                return {"status": "error", "message": "Failed to update implementation plan"}
    ...
    async def check_task(
        ...
        try:
            ...
            if not success:
                return {"status": "error", "message": f"Failed to check task {task_number}"}
    ...
    async def uncheck_task(
        ...
        try:
            ...
            if not success:
                return {"status": "error", "message": f"Failed to uncheck task {task_number}"}
    ...
    async def bulk_check_tasks(
        ...
        try:
            ...
            msg = (
                f"Completed {len(completed)} tasks" + (f", {len(errors)} failed" if errors else "")
            )
            return {
                "status": status,
                "spec_id": spec_id,
