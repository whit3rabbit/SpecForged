...
class PlanGenerator:
    ...
    def _generate_from_design(self, design: Dict, user_stories: List[UserStory]) -> List[Task]:
        ...
        if 'architecture' in design:
            arch_task = self._create_task(
                ...
                description=design['architecture'][:200] + "..." if len(design['architecture']) > 200 else design['architecture']
            )
        ...
        if 'components' in design:
            for component in design['components']:
                comp_task = self._create_task(
                    ...
                    description=component.get('description', f"Implementation of {component['name']} component")
                )
    ...
    def _create_task(self, title: str, description: str, linked_requirements: List[str] = None) -> Task:
    ...
    def _create_subtask(self, parent_task: Task, title: str, description: str, linked_requirements: List[str] = None) -> Task:
    ...
    def _number_tasks_hierarchically(self, tasks: List[Task]) -> None:
        ...
        for task in tasks:
            ...
            for subtask in task.subtasks:
                ...
                self._number_subtasks_recursively(subtask, f"{main_task_number}.{subtask_number}")
    ...
    def _set_task_dependencies(self, tasks: List[Task]) -> None:
        ...
        setup_tasks = [t for t in all_tasks if 'setup' in t.title.lower() or 'structure' in t.title.lower()]
    ...
    def update_plan_from_spec_changes(self, spec: Specification, existing_tasks: List[Task]) -> List[Task]:
        ...
        existing_task_map = {task.title: task for task in self._flatten_tasks(existing_tasks)}
