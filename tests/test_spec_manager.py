...
def test_add_ears_requirement(temp_spec_dir):
    ...
    manager = SpecificationManager(temp_spec_dir)
    spec = manager.create_specification("Test Feature", "A test feature")
    story = manager.add_user_story(spec.id, "user", "to login", "I can access my account")
    ...
    assert "WHEN a user enters valid credentials THE SYSTEM SHALL authenticate and create a session" == ears_string
...
def test_markdown_generation(temp_spec_dir):
    ...
    spec = manager.create_specification("Test Feature", "A test feature")
    story = manager.add_user_story(spec.id, "user", "to login", "I can access my account")
    manager.add_ears_requirement(spec.id, story.id, "WHEN login submitted", "validate credentials")
