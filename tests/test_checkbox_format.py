...
from src.specforged.models import Specification, SpecStatus, WorkflowPhase, UserStory, EARSRequirement, Task
...
def test_checkbox_markdown_generation(spec_manager, sample_spec_with_tasks, temp_spec_dir):
...
def test_get_task_by_number(spec_manager, sample_spec_with_tasks):
    ...
    if main_task_with_subtasks and main_task_with_subtasks.subtasks:
        ...
        found_subtask = spec_manager.get_task_by_number(sample_spec_with_tasks, subtask.task_number)
...
def test_update_implementation_plan_preserves_status(spec_manager, sample_spec_with_tasks):
...
def test_tasks_file_progress_summary(spec_manager, sample_spec_with_tasks, temp_spec_dir):
...
def test_invalid_task_operations(spec_manager, sample_spec_with_tasks):
    ...
    assert spec_manager.get_task_by_number(sample_spec_with_tasks, fake_task_number) is None


def test_nested_subtask_hierarchy_in_markdown(spec_manager, sample_spec_with_tasks, temp_spec_dir):
    ...
    if spec.tasks:
        ...
        if main_task.subtasks:
            ...
            main_line = next((line for line in lines if main_task.task_number in line and main_task.title in line), None)
            sub_line = next((line for line in lines if subtask.task_number in line and subtask.title in line), None)
