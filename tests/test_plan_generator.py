...
from src.models import Specification, UserStory, EARSRequirement, SpecStatus, WorkflowPhase
...
def sample_spec():
    ...
    spec = Specification(
        ...
        design={
            ...
            "components": [
                {"name": "AuthController", "description": "Handles authentication requests"},
...
def test_generate_implementation_plan(plan_generator, sample_spec):
    ...
    main_task_numbers = [task.task_number for task in tasks if '.' not in task.task_number]
...
def test_generate_from_design(plan_generator, sample_spec):
    ...
    tasks = plan_generator._generate_from_design(sample_spec.design, sample_spec.user_stories)
    ...
    component_tasks = [title for title in task_titles if "authcontroller" in title or "usermodel" in title]
    assert len(component_tasks) > 0
    
    # Check for data model task
    data_tasks = [title for title in task_titles if "data" in title and "model" in title]
...
def test_get_task_by_number(plan_generator, sample_spec):
    ...
    if any(task.subtasks for task in tasks):
        ...
        found_subtask = plan_generator.get_task_by_number(tasks, first_subtask.task_number)
...
def test_update_plan_from_spec_changes(plan_generator, sample_spec):
    ...
    # Update plan
    updated_tasks = plan_generator.update_plan_from_spec_changes(sample_spec, initial_tasks)
...
def test_set_task_dependencies(plan_generator, sample_spec):
    ...
    setup_tasks = [task for task in flat_tasks if 'setup' in task.title.lower() or 'structure' in task.title.lower()]
